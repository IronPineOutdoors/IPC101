...
def run():
    ...
    scad=(g.ROOT/'branding/iron_pine_tree_trace.scad').read_text()
    polys=[]
    ...
    report={'revision':'N1.2','status':'CAD verified; physical print pending',
        ...
        'trace':json.loads((g.ROOT/'branding/tree_trace_verification.json').read_text()),
    ...
    (g.ROOT/'N12_faceplate_verification.json').write_text(json.dumps(report,indent=2)+'\n')
