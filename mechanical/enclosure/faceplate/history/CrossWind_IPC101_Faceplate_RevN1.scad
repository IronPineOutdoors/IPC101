// N1: operator-view button positions measured on the physical PCB.
...
module pine(x,y,s=1){
 // Separate trunk and branch silhouette; bottom y=83.54, above OUTDOORS.
 translate([x,y,0]) linear_extrude(gd) scale([s,s]) polygon(points=[
 [0,10],[-4.5,3],[-2.5,3],[-6,-1],[-3.5,-1],[-7,-4.5],
 [-1.2,-4.5],[-1.2,-7],[1.2,-7],[1.2,-4.5],[7,-4.5],
 [3.5,-1],[6,-1],[2.5,3],[4.5,3]]);
}
module swoosh(){
...
module desired_operator_graphics(){
 // M4 branding arrangement with corrected tree; labels track button centers.
 txt("IRON",17.5,89,3.7);
 txt("PINE",49,89,3.7);
 pine(34,89,.78);
 txt("OUTDOORS",34,81.5,2.0,"center","Liberation Sans");
 line2d(9,81.5,19,81.5,.65);
 line2d(49,81.5,59,81.5,.65);

 txt("CROSSWIND",112,88,4.3);
 swoosh();

 txt("ARM",arm[0],31,3.0);
 txt("PULL",pull[0],31,3.0);
}
...
module raw_white(){
 ...
 text("IPC101-FP-N1",size=3,font="Liberation Sans:style=Bold",halign="center");}
}
