...
def run():
    ...
    for material,p in [('WHITE_PETGHF',white),('BLACK_AMS',black)]:
        mesh=trimesh.load_mesh(asset(g.ROOT, f'CrossWind_IPC101_Faceplate_RevN1_{material}.stl'))
    ...
    (asset(g.ROOT, 'N1_faceplate_verification.json')).write_text(json.dumps(report,indent=2)+'\n')
