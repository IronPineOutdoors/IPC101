...
def run():
    ...
    for name in ('Box','Bracket','Wiring_Plate'):
        for orientation in ('INSTALLED','PRINT'):
            mesh=trimesh.load_mesh(asset(g.SOURCE_ROOT, f'CrossWind_IPC101_{name}_RevH_{orientation}.stl'))
            if name=='Bracket':
    ...
    for name,part in parts.items():
        ...
        for orientation in ('INSTALLED','PRINT'):
            mesh=trimesh.load_mesh(asset(g.ROOT, f'CrossWind_IPC101_{name}_RevI_{orientation}.stl'))
