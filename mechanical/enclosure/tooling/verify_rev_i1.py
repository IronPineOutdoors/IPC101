...
def exported_checks(parts):
    ...
    for name,part in parts.items():
        ...
        for orientation in ('INSTALLED','PRINT'):
            mesh=trimesh.load_mesh(asset(g.ROOT, f'CrossWind_IPC101_{name}_RevI1_{orientation}.stl'))
...
def run():
    ...
    original=trimesh.load_mesh(asset(g.ROOT, 'IPC101_RevA_RearCarrier_R3.stl'))
    ...
    (asset(g.ROOT, 'RevI1_verification.json')).write_text(json.dumps(report,indent=2)+'\n')
