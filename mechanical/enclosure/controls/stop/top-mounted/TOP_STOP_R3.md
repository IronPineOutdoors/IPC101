...
Release coupon R3 PRINT (retired export; recover from Git history)
...
If release is comfortable in both orientations, print Roof R3 PRINT (retired export; recover from Git history). Do not use the old R1/R2 roof for the next full print. Keep the existing base and roof hardware. Inspect the full roof's support preview separately; the no-support instruction applies only to the simplified coupon.
