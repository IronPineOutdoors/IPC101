...
if __name__=='__main__':
    ...
    for label in ('ARM','PULL'):
        for part,material in [('white','WHITE_PETGHF'),('black','BLACK_AMS')]:
            subprocess.run([args.openscad,'-o',str(root/f'CrossWind_IPC101_{label}_Cap_R1_{material}.stl'),
                '-D',f'label="{label}"','-D',f'part="{part}"',str(root/'CrossWind_IPC101_Button_Caps_R1.scad')],check=True)
