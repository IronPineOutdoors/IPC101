...
arm=[33.5,20.75]; pull=[103.5,20.75]; led=[68.5,31]; nav=[45,63.5]; oled=[103.5,63.5];
...
module pine(x,y,s=1){
 linear_extrude(gd) polygon(points=[
 [x,y+10*s],[x-5.8*s,y-5*s],[x-1.7*s,y-2*s],
 [x-7*s,y-8*s],[x-1.2*s,y-5*s],[x,y-10*s],
 [x+1.2*s,y-5*s],[x+7*s,y-8*s],[x+1.7*s,y-2*s],
 [x+5.8*s,y-5*s]
 ]);
}
module swoosh(){
...
module desired_operator_graphics(){
 // EXACT approved preview arrangement
 ...
 txt("ARM",33.5,31,3.0);
 txt("PULL",103.5,31,3.0);
}

// Proven underside-view conversion from Rev M: mirror complete design about X=75.
